...
class View extends Container {
	...
	public function render() {
		...
		if (!$argumentCount) {
			throw new Exception('BGStudios\\Component\\Mvc\\View Error: the render() method requires a path to a template.');
		}
		else {
			$path = $arguments[0];
			$data = $argumentCount > 1 ? $arguments[1] : Array();
			if (!$path) {
				throw new Exception('BGStudios\\Component\\Mvc\\View Error: render() method $path argument cannot be blank');
			}
